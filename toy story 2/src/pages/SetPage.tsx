  ...
  return (
    <div className="bg-white min-h-screen flex flex-col">
      ...
      <main className="flex-1 w-full max-w-[1000px] mx-auto px-4 py-8">
        <h1 className="font-rowdies text-2xl text-gray-900 mb-8 border-b border-gray-100 pb-4">
          Bộ sưu tập đặc biệt
        </h1>
        ...
        {loading ? (
          <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-[200px] bg-gray-50 animate-pulse rounded-2xl border border-gray-100"
              />
            ))}
          </div>
        ) : sets.length === 0 ? (
          <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            <p className="font-red-hat text-gray-500">
              Chưa có bộ sưu tập nào được mở bán.
            </p>
          </div>
        ) : (
          <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
              ...
              >
                <div className="flex gap-6">
                  ...
                  <div className="min-w-0 flex-1 flex flex-col justify-between">
                    <div>
                      <h2 className="font-tilt-warp text-lg text-gray-900 group-hover:text-[#ca002a] transition-colors line-clamp-1">
                        {s.name ?? "—"}
                      </h2>
                      {s.description && (
                        <p className="font-red-hat text-sm text-gray-600 mt-2 line-clamp-2 leading-relaxed">
                          {s.description}
                        </p>
                      )}
                    </div>
