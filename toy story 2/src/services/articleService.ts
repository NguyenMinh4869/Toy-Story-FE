...
// Initialize Contentful Client only if credentials are provided to avoid runtime errors
...
export const getArticles = async (): Promise<ViewArticleDto[]> => {
  if (!client) {
    console.warn('Contentful client not initialized. Please set VITE_CONTENTFUL_SPACE_ID and VITE_CONTENTFUL_ACCESS_TOKEN in your .env file.');
    return [];
  }
  try {
