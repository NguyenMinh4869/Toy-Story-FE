 ...
 */
export const getDeactivatePreview = async (
  brandId: number,
): Promise<BrandActionPreviewDto> => {
  const response = await apiGet<{
    statusCode: number;
    message: string;
    data: BrandActionPreviewDto;
 ...
 */
export const getReactivatePreview = async (
  brandId: number,
): Promise<BrandActionPreviewDto> => {
  const response = await apiGet<{
    statusCode: number;
 ...
 */
export const reactivateBulk = async (
  payload: BrandReactivateBulkDto,
): Promise<{ statusCode: number; message: string }> => {
  const response = await apiPost<{ statusCode: number; message: string }>(
    "/brands/reactivate-bulk",
    payload,
  );
  return response.data;
};
