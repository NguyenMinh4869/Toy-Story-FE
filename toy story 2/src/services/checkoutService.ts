...
export const syncCartToServer = async (
  ...
  for (const item of items) {
    if (item.productId && item.quantity > 0) {
      await apiPut(
        `/carts/items?productId=${item.productId}&quantity=${item.quantity}`,
      );
    }
  }
};
...
export const validateVoucher = async (
  ...
  const response = await apiPost<ValidateVoucherResponse>(
...
export const createPayment = async (
  ...
  try {
    ...
    const fallbackResponse = await apiPost<CreatePaymentResponse>(
      "/payment/create",
      payload,
    );
...
export const paymentSuccess = async (orderCode: number): Promise<any> => {
  try {
    const response = await apiPut<any>(`/checkout/success/${orderCode}`);
    return response.data;
  } catch (error) {
    const apiError = error as ApiError;
    if (apiError.status !== 404) {
...
export const paymentCancel = async (orderCode: number): Promise<any> => {
  try {
    const response = await apiPut<any>(`/checkout/cancel/${orderCode}`);
    return response.data;
  } catch (error) {
    const apiError = error as ApiError;
    if (apiError.status !== 404) {
